        ...
        {data.map((short, key) => (
          <Link to={`/watch?v=${short.videoId}`} key={key} className="group">
            <div className="w-48 h-80 relative overflow-hidden rounded-lg bg-zinc-900">
              <img src={short.thumbnail[0]?.url} className="size-full object-cover group-hover:scale-105 transition" />

              <div className="absolute bottom-0 right-0 left-0 bg-linear-to-t from-black/80 to-transparent p-4">
