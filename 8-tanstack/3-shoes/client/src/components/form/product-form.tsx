...
const ProductForm: FC<Props> = ({ data, isPending, mutate }) => {
  ...
  return (
    <Formik initialValues={initialValues} validationSchema={PRODUCT_SCHEMA} onSubmit={handleSubmit} enableReinitialize>
      <Form className="flex flex-col gap-5">
        {INPUT_ARRAY.map((input) => (
          <FormField {...input} />
        ))}

        <button disabled={isPending} className="bg-blue py-1 px-4 rounded-md text-white hover:bg-blue/80">
          Gönder
        </button>
      </Form>
    </Formik>
