          ...
          <tbody>
            {data?.map((product) => (
              <tr className="bg-white border-b border-zinc-300 hover:bg-gray-50 font-semibold">
                <td className="p-3">
                  <img src={product.picture[0]} className="size-16 md:size-18 max-w-full max-h-full rounded-xl" />
                </td>

                <td className="row">{product.name.length > 25 ? product.name.slice(0, 25) + "..." : product.name}</td>
                <td className="row">${product.price}</td>
                <td className="row">{product.discount > 0 ? `%${product.discount}` : "Yok"}</td>
