        ...
        {SIZES.map((size, key) => {
          ...
          return (
            <button
              ...
              disabled={!inStock}
              onClick={() => toggle(size)}
              className={`py-2 px-4 rounded-md cursor-pointer transition hover:bg-zinc-400 disabled:bg-[#d2d1d3] disabled:text-[#8f8x91]  ${selected == size ? "bg-black text-white" : "bg-white text-black"}`}
            >
              {size}
            </button>
          );
